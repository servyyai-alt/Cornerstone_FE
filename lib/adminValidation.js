...
const isValidSiteId = (value) => /^[A-Za-z0-9_-]+$/.test(String(value || '').trim());

const isValidGoogleAnalyticsId = (value) => /^G-[A-Z0-9]{10}$/.test(String(value || '').trim());

const isValidGoogleTagManagerId = (value) => /^GTM-[A-Z0-9]{7}$/.test(String(value || '').trim());

export const validateSiteSettingsForm = (form) =>
  normalizeErrors([
    isNonEmpty(form.siteName) ? '' : 'Site name is required.',
    isValidHttpUrl(form.siteUrl) ? '' : 'Site URL must be a valid http(s) link.',
    isNonEmpty(form.siteDescription) ? '' : 'Site description is required.',
    form.siteLogo ? (form.siteLogo.startsWith('/') || isValidHttpUrl(form.siteLogo) ? '' : 'Site logo must be a valid URL or path.') : '',
    form.favicon ? (form.favicon.startsWith('/') || isValidHttpUrl(form.favicon) ? '' : 'Favicon must be a valid URL or path.') : '',
    form.googleSiteVerification ? (String(form.googleSiteVerification).trim().length <= 200 ? '' : 'Google verification code is too long.') : '',
    form.bingSiteVerification ? (String(form.bingSiteVerification).trim().length <= 200 ? '' : 'Bing verification code is too long.') : '',
    form.yandexSiteVerification ? (String(form.yandexSiteVerification).trim().length <= 200 ? '' : 'Yandex verification code is too long.') : '',
    form.googleAnalyticsId ? (isValidGoogleAnalyticsId(form.googleAnalyticsId) ? '' : 'Google Analytics ID must look like G-XXXXXXXXXX.') : '',
    form.googleTagManagerId ? (isValidGoogleTagManagerId(form.googleTagManagerId) ? '' : 'Google Tag Manager ID must look like GTM-XXXXXXX.') : '',
    form.facebookPixelId ? (isValidSiteId(form.facebookPixelId) ? '' : 'Facebook Pixel ID contains invalid characters.') : '',
    form.microsoftClarityId ? (isValidSiteId(form.microsoftClarityId) ? '' : 'Microsoft Clarity ID contains invalid characters.') : '',
    ...
    form.twitterUrl ? (isValidHttpUrl(form.twitterUrl) ? '' : 'Twitter URL must be a valid http(s) link.') : '',
    ...
    isNonEmpty(form.supportPhone) ? (isValidPhone(form.supportPhone) ? '' : 'Support phone is invalid.') : '',
  ]);
