...
export const normalizeSiteSettings = (settings = {}) => {
  ...
  return {
    ...
    chatbotUrl: trimValue(settings.chatbotUrl),
...
export const createPageMetadata = async ({
  ...
  const metaTitle = seoPage?.seoTitle || seoPage?.metaTitle || (title && title !== siteConfig.name ? title : siteName);
  ...
  const metaKeywords = normalizeKeywords(
    seoPage?.seoKeywords?.length
      ? seoPage.seoKeywords
      : seoPage?.metaKeywords?.length
        ? seoPage.metaKeywords
        : keywords.length
          ? keywords
          : defaultKeywords
  );
  const ogTitle = seoPage?.ogTitle || seoPage?.openGraphTitle || metaTitle;
  const ogDescription = seoPage?.ogDescription || seoPage?.openGraphDescription || metaDescription;
